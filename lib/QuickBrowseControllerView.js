...
export default class QuickBrowseControllerView extends React.Component {
  ...
  componentWillMount(): void {
    this._navigate(this.props.initialDirectory, this.props.initialEntryName);
  }
  ...
  /**
   * Navigate to a different directory, and optionally jump to a specific named
   * entry.
   */
  _navigate(directory: Directory, initial?: string) {
    ...
    FileSystem.getEntries(directory).then(entries => {
      ...
      const orderedEntries = dirs.concat(files);

      // Set the initial index.
      let selected = 0;
      if (initial !== null) {
        orderedEntries.forEach((value, idx) => {
          if (value.getBaseName() == initial) {
            selected = idx;
          }
        });
      }
  ...
  _handleParent(): void {
    this._navigate(
      this.state.currentDirectory.getParent(),
      this.state.currentDirectory.getBaseName(),
    );
  }
